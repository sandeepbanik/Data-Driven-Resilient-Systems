...
for i=L:-1:1
    ...
    if (F + E*W)^2*P_1(:,i+1) >= (A - B*K)^2*P_0(:,i+1) + max(D,A) 
        ...
        P_0(:,i) = Q_d + D + (F - B*K)^2*P_0(:,i+1) - D*A/((F + E*W)^2*P_1(:,i+1) - (F-B*K)^2*P_0(:,i+1));
    ...
    if (F + E*W)^2*P_1(:,i+1) >= (F - B*K)^2*P_0(:,i+1) + max(D,A)
        ...
        P_1(:,i) = Q_a - A + (F + E*W)^2*P_1(:,i+1) + D*A/((F + E*W)^2*P_1(:,i+1) - (F-B*K)^2*P_0(:,i+1));
